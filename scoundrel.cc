...
ScoundrelGame::ScoundrelGame(uint32_t seed) {
  ...
  this->_deck = std::make_unique<CardStack>(this->_starting_deck());
  _shuffle();

  this->_discard = std::make_unique<CardStack>(CardStack());
  this->_equipped_weapon = std::nullopt;
  this->_killed_monsters = std::make_unique<CardStack>(CardStack());
  this->_room = std::make_unique<CardStack>(CardStack());
...
int32_t ScoundrelGame::get_health() const { return this->_health_points; }

CardStack *ScoundrelGame::get_room() const { return this->_room.get(); }

CardStack *ScoundrelGame::get_deck() const { return this->_deck.get(); }

CardStack *ScoundrelGame::get_discard() const { return this->_discard.get(); }
...
bool ScoundrelGame::has_died() const { return this->_health_points <= 0; }

bool ScoundrelGame::has_exited_dungeon() const {
  return std::count_if(this->_discard->begin(), this->_discard->end(),
                       [](Card card) {
...
bool ScoundrelGame::can_drink_potion() const { return this->_can_heal; }
...
bool ScoundrelGame::can_fight_monster_with_weapon_at(
    uint32_t room_index) const {
  ...
  if (this->has_weapon()) {
    ...
    if (monster_card.suit == CardSuit::CLUBS ||
        monster_card.suit == CardSuit::SPADES) {
...
void ScoundrelGame::fight_monster_barehanded_at(uint32_t room_index) {
  ...
  if (monster.suit != CardSuit::SPADES && monster.suit != CardSuit::CLUBS) {
    throw std::invalid_argument(
        "Cannot fight monster if the card is not a monster!");
  }
...
void ScoundrelGame::fight_with_weapon_at(uint32_t room_index) {
  ...
  this->_health_points -= std::max(
      (int32_t)monster.rank - (int32_t)this->_equipped_weapon.value().rank, 0);
...
void ScoundrelGame::drink_potion_at(uint32_t room_index) {
  ...
  if (this->_can_heal) {
    this->_health_points =
        std::min(this->_health_points + potion.rank, STARTING_HEALTH);
  } else {
